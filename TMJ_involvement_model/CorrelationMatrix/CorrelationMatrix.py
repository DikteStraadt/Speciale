...
def make_previous_status_correlation_matrix(df_previous_status):
    ...
    result_df = result_df.drop(columns=['Suffix', 'Base', 'ID', 'previousstatus', 'last status'], axis=1).sort_values(by='index').drop(columns=['index'], axis=1)
    result_df_0 = result_df_0.drop(columns=['Suffix', 'Base', 'ID', 'previousstatus', 'last status'], axis=1).sort_values(by='index').drop(columns=['index'], axis=1)
    result_df_1 = result_df_1.drop(columns=['Suffix', 'Base', 'ID', 'previousstatus', 'last status'], axis=1).sort_values(by='index').drop(columns=['index'], axis=1)
    result_df_2 = result_df_2.drop(columns=['Suffix', 'Base', 'ID', 'previousstatus', 'last status'], axis=1).sort_values(by='index').drop(columns=['index'], axis=1)
